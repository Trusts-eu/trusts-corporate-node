...
ckan config-tool $CKAN_INI "debug.remote.host.ip = $DEBUG_SERVER_HOST"
...
if [[ "$CKAN__PLUGINS" =~ .*"trusts_dev".* ]]; then
  echo "Enabling pass trough so the pydev debugger can work..."
  sudo -u ckan -EH ckan -c $CKAN_INI run -H 0.0.0.0 -E
else
  echo "Default run of the development server"
